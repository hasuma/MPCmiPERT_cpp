...
int main(){
	...
	if (parametros_sistema.is_open())
	{
		...
		parametros_sistema>> yref(1,0);	
		parametros_sistema>> yref(2,0);             // Theta4 reference value
	}
	else{
	...
	cout << "Referencia: "<< endl << yref<<  endl;
	...
	for(int i = 0; i<kmax;i++){
		...
		ruido(2,i) = ((double)((rand()%5001)-2500))/((double)100000);
		ruido(3,i) = ((double)((rand()%2801)-1400))/((double)100000);
		ruido(4,i) = ((double)((rand()%28001)-14000))/((double)100000);
		ruido(5,i) = ((double)((rand()%28001)-14000))/((double)100000);
	}
	...
	for(int k = 0; k<kmax;k++){
		...
		xAux = {x(0,k),
			...
			x(8,k),
		     	x(9,k)};
		xAux = xAux.t();
		y.col(k) = xAux.rows(0,2);
		xAux = xAux + ruido.col(k);
		ym.col(k) = xAux.rows(0,2);
		//**********************************************************
		//up.col(k) = u0;
