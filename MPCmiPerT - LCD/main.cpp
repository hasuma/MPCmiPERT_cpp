...
class fnlin{
	...
	double g = 9.81;
    	double l2 = 0.173;
    	double Ix4 = 0.01*3;
    	double Iy2 = 0.306954*3;
    	double Iz1 = 0.28761702*6;
    	double b1 = 0.215;
   	double b2 = 0.215;
...
int main(){
	...
	mat chiKK = zeros<mat>(numStates+numRef,kmax);
	// Entrada
	...
	x(3,0) = up0[0];
	x(4,0) = up0[1];
	mat aux_r = zeros<mat>(numStates+numRef,3);
	...
	for(int k = 0; k<kmax;k++){
		...
		//**********************************************************
		//up.col(k) = u0;
		aux_r = controle.INPUT(xAux.col(k),ym.col(k));
		chiKK.col(k) = aux_r.submat(0,0,numStates+numRef-1,0);
		uAux = aux_r.submat(0,1,numInput-1,2);		
		ulqr.col(k) = uAux.col(0);
		umpc.col(k) = uAux.col(1);
		u.col(k) = ulqr.col(k) + umpc.col(k);
		
		up.col(k) = u.col(k) + u0;
		du.col(k) = up.col(k) - ukm1;
