...
public:
	...
	void operator()(const vector<double> &x, vector<double> &dxdt, const double t){
		...
		//**********************************************************
		//Dinamica Bancada linear
		mat q = {0,0,-pi*0.5,x[0]};
		mat qdot = {0,0,0,x[3]};
...
int main(){
	...
	if (parametros_sistema.is_open())
	{
		...
		parametros_sistema>> yref(0,0);             // Theta4 reference value
	}
	else{
	...
	cout << "Posicao inicial: "<< posIni(0,0)<<  endl;
	cout << "Referencia: "<< yref(0,0)<<  endl;
	...
	Control controle(numStates, numInput, numRef, yref, Ts, up0);
	...
	for(int k = 0; k<kmax;k++){
		...
		xAux = {x(0,k),
		     	x(3,k)};
		xAux = xAux.t();
		y.col(k) = xAux.rows(0,0);
		xAux = xAux + ruido.col(k);
		ym.col(k) = xAux.rows(0,0);
		//**********************************************************
